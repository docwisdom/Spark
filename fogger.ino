...
#define SMTP_LOCATION "Laser Tag"
    ...
    //if (Time.hour() == 8 && Time.minute() == 0 && Time.second() == 0) {
        ...
        if (digitalRead(D0) == HIGH) {
            if (sentOnce == 0) {
                ...
                RGB.color(250, 0,0);
                TANK = 1;
                sentOnce = SendEmail(SMTP_SERVER, SMTP_USER_BASE64, SMTP_PASS_BASE64, SMTP_FROM_EMAIL, SMTP_TO_EMAIL, SMTP_SUBJECT, SMTP_BODY_LOW, SMTP_LOCATION);
                if (sentOnce) {
    	            RGB.color(0, 250, 0);
    	            delay(2000);
    	            sentOnce = 0;
        ...
        if (digitalRead(D2) == HIGH) {
            if (sentOnce == 0) {
                ...
                RGB.color(250, 0,0);
                TANK = 1;
                sentOnce = SendEmail(SMTP_SERVER, SMTP_USER_BASE64, SMTP_PASS_BASE64, SMTP_FROM_EMAIL, SMTP_TO_EMAIL, SMTP_SUBJECT, SMTP_BODY_EMPTY, SMTP_LOCATION);
                if (sentOnce) {
    	            RGB.color(0, 250, 0);
    	            delay(2000);
    	            sentOnce = 0;
 ...
 * Thank you @jboswell !
 */
int SendEmail(char *smtpServer, char *username, char *password, char *fromEmail, char *toEmail, char *subject, char *body, char *location) {
    ...
    echoSocketWrite(client, String("subject: ") + String(location) + String(subject) + String("\r\n"));
    flushToSerial(client); //client.flush();

    echoSocketWrite(client, "\r\n");
    echoSocketWrite(client, String("The fluid is ") + String(body) + String(" in fogger unit "));
    //echoSocketWrite(client, unit);
    echoSocketWrite(client, "\r\n");
    echoSocketWrite(client, ".");
    echoSocketWrite(client, "\r\n");
    echoSocketWrite(client, "\r\n");
    ...
    echoSocketWrite(client, "\r\n");
    if (blockForResponse("221", 2500) == 1) {
